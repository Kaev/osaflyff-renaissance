...
namespace FlyffWorld
{
    ...
    public class Awakening
    {
        ...
        /*public static AwakeAttribute[] ToAwakeAttributeArray(ulong awakening)
        {
        ...
        //[Divinepunition] : Omnio awake extraction method
        public static AwakeAttribute[] ToAwakeAttributeArray(ulong awakening)
        {
            List<AwakeAttribute> attribs = new List<AwakeAttribute>(3);
            awakening = awakening >> 8;
            while (true)
            {
                AwakeAttribute attribute = new AwakeAttribute();
                attribute.value = (int)awakening & 511;
                awakening = awakening >> 9;
                attribute.negative = ((awakening & 1) == 1);
                awakening = awakening >> 1;
                attribute.attribute = (byte)(awakening & 127);
                awakening = awakening >> 7;
                attribs.Add(attribute);
                if (awakening != 0)
                {
                    // There are other attributes.
                    awakening = awakening >> 1;
                    awakening = awakening & 0;
                }
                else
                {
        ...
        public static ulong Generate()
        {
            ...
            for (int i = awakeningCount-1; i >= 0; i--)
            {
        ...
        public static ulong ToInt64(List<AwakeAttribute> a)
        {
            ulong b = 0;
