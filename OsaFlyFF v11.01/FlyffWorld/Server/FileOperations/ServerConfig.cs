...
namespace FlyffWorld
{
    class Server // instead of using the long name, this one is acceptable IMO
    {
        ...
        public static int exp_rate, penya_rate, drop_rate;
        static string notes = "";
        ...
        public static void Refresh()
        {
            ...
            exp_rate = worldrates.ReadInt("exp_rate", 1);
            drop_rate = worldrates.ReadInt("drop_rate", 1);
            penya_rate = worldrates.ReadInt("penya_rate", 1);
            worldrates.destroy();
            Log.Write(Log.MessageType.info, "*** SERVER RATES: {0}x EXP, {1}x DROP, {2}x PENYA ***", exp_rate, drop_rate, penya_rate);
