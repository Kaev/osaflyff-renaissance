...
namespace FlyffWorld
{
    public partial class WorldThreads
    {
        public void MoverThreadProcess()
        {
                        ...
                        for (int i = 0; i < WorldServer.world_drops.Count; i++) // Nicco->Drops
                        {
                            ...
                            lock (drop.moverLock)
                            {
                                ...
                                if (!drop.neverDespawn) // If true the item always lays on floor
                                {
                                    ...
                                    {
                                        for (int x = drop.c_spawns.Count; x >0 ; x--)
                                        {
            ...
            {
                Log.Write(Log.MessageType.fatal, "Mover thread terminated due to an exception.");
                Log.Write(Log.MessageType.fatal, "Error message: " + e.Message);
                Log.Write(Log.MessageType.fatal, "Stack trace: " + e.StackTrace);
                Log.Write(Log.MessageType.notice, "Restarting mover thread.");
