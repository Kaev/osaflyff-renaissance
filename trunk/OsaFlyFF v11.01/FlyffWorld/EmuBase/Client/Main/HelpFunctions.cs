    ...
    public partial class Client
    {
        ...
        public Friend BuildFriend(int id, int blocked)
        {
            ...
            else
            {
                ...
                friend.dwNetworkStatus = NetworkStatus.Offline;
            }
            return friend;
        }
        [Obsolete]
        ...
        public void ResetUserAttributes()
        {
            ...
            for (int attributeID = 1; attributeID < 150; attributeID++)
            {
                ...
                if (r_attributeID > 94)
                    r_attributeID += 9906;
                if (r_attributeID != 38 && r_attributeID != 39 && r_attributeID != 40)
                {
                    diffAttribute = c_totalAttributes[r_attributeID] - _lastAttributes[r_attributeID];
                    if (diffAttribute > 0)
                        SendPlayerAttribRaise(r_attributeID, diffAttribute,-1);
                    if (diffAttribute < 0)
                        SendPlayerAttribDecrease(r_attributeID, 0 - diffAttribute);
                }
            }
            if (c_attributes[DST_HP] > c_data.f_MaxHP)
                SendPlayerAttribSet(DST_HP, c_data.f_MaxHP);
            if (c_attributes[DST_MP] > c_data.f_MaxMP)
                SendPlayerAttribSet(DST_MP, c_data.f_MaxMP);
            if (c_attributes[DST_FP] > c_data.f_MaxFP)
                SendPlayerAttribSet(DST_FP, c_data.f_MaxFP);
        }
