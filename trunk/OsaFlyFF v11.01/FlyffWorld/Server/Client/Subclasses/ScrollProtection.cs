    ...
    public class ScrollProtection
    {
        ...
        public bool SaveData()
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 7; i++)
                    if (bProtections[i])
                        Database.Execute("INSERT INTO `flyff_activescrolls` (`charid`,`scrollid`) VALUES ('{0}','{1}')",
                            dwCharID, i);
            }
            return ret;
        }
        ...
        public void sendIcon(Client player, int scroll) // TODO: Atm can't debuff icon because unknown packet!
        {
            ...
            pak.Addshort(0); // buff type
            switch (scroll)
            {
                case SCROLL_APROTECT: pak.Addshort(26473); break;
                case SCROLL_BLESSING: pak.Addshort(10430); break;
                case SCROLL_GPROTECT: pak.Addshort(10465); break;
                case SCROLL_HSMELTING: pak.Addshort(26203); break;
                case SCROLL_SMELTING: pak.Addshort(10468); break;
                case SCROLL_SPROTECT: pak.Addshort(10464); break;
                case SCROLL_XPROTECT: pak.Addshort(10488); break;
                default: return; break;
            }
            pak.Addint(0); // buff level
            pak.Addint(1); // buff time
            pak.Send(player);
        }
    }
}
